...
def test_missing_bpspx_fails_closed_without_direction():
    result = build_breadth_regime({"ticker": "SPX"})
...
def test_sub_15_falling_is_capitulation_not_buy_signal():
    result = build_breadth_regime({"bpspx": 14.0, "bpspx_previous": 18.0})
...
def test_rising_from_extreme_is_early_not_confirmed_recovery():
    result = build_breadth_regime({"bpspx": 19.0, "bpspx_previous": 14.0})
...
def test_cross_above_30_confirms_recovery():
    result = build_breadth_regime({"bpspx": 32.0, "bpspx_previous": 27.0})
...
def test_routes_expose_dashboard_payload():
    app = Flask(__name__)
    register_breadth_regime_routes(app, last_result_provider=lambda: {"bpspx": 14, "bpspx_previous": 18})
