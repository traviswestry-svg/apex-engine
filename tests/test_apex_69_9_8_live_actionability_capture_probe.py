...
def test_runtime_probe_records_capture_source_immediately(tmp_path, monkeypatch):
    ...
    assert runtime["last_actionability_capture_version"] == "69.9.8"
...
def test_release_truth_routes_and_guardrails_are_69_9_8():
    ...
    assert manifest["apex_version"] == manifest["semantic_version"] == manifest["application_version"] == "69.9.8"
    assert manifest["build_name"] == "Live Actionability Capture Probe & Lifecycle Attribution Closure"
    ...
    assert "apex_version: 69.9.8" in registry
