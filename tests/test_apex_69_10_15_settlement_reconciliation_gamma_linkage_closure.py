...
def test_release_scoped_gamma_linkage_does_not_reconstruct(tmp_path):
    ...
    with ep._connect(db) as c:
        ...
        c.execute("INSERT INTO decisions(decision_id,observed_at,ticker,learning_eligible,snapshot_json,canonical_gamma_snapshot_id) VALUES(?,?,?,?,?,?)",('new-linked','2026-09-21T13:30:00Z','SPX',1,json.dumps({'apex_release_version':'69.10.19'}),'g1'))
        c.execute("INSERT INTO decisions(decision_id,observed_at,ticker,learning_eligible,snapshot_json,canonical_gamma_snapshot_id) VALUES(?,?,?,?,?,?)",('new-missing','2026-09-21T13:31:00Z','SPX',1,json.dumps({'apex_release_version':'69.10.19'}),None))
