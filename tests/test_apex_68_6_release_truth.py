...
def test_release_manifest_ratcheted_to_68_6():
    data = json.loads(Path('config/apex_release_manifest.json').read_text())
    assert data['apex_version'] == '68.6.0'
    assert data['build_name'] == 'Decision Outcome Attribution & Abstention Effectiveness'
    g = data['guardrails']
...
def test_capability_registry_declares_effectiveness_surface():
    ...
    assert 'apex_version: 68.6.0' in text
