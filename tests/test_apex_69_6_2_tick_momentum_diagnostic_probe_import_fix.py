...
def test_69_6_2_release_truth():
    ...
    assert manifest["apex_version"] == manifest["semantic_version"] == manifest["application_version"] == "69.6.2"
    assert manifest["guardrails"]["tick_momentum_diagnostic_probe_urlparse_import_present"] is True
    registry = (ROOT / "config/apex_capability_registry.yaml").read_text()
    assert "apex_version: 69.6.2" in registry
    assert 'version: "69.6.2"' in registry
