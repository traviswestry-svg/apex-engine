...
const AUTO_INTERVAL = 15000; // cache poll only; heavy composition is background-only
let osRetryTimer = null;
...
async function loadOS() {
  ...
  try {
    ...
    if (!data) {
      ...
      if (osRetryTimer) clearTimeout(osRetryTimer);
      osRetryTimer = setTimeout(() => { osRetryTimer = null; loadOS(); }, 5000);
      return;
    }
    ...
    if (errEl) {
      if (apiResult.warming) {
        errEl.style.display = '';
        errEl.textContent = '⌛ Engines warming in the background — dashboard remains available';
      } else if (apiResult.stale || data.stale) {
        errEl.style.display = '';
        errEl.textContent = '⟳ Showing cached data — background refresh in progress';
      } else {
