...
def _node(snapshot: Mapping[str, Any], engine: str, aliases: Tuple[str, ...], weight: float, now: float) -> EvidenceNode:
    ...
    # Independent-evidence discount: a source that is really one repeated burst
    # (e.g. a single sweep re-hitting the tape) reports independent_evidence_factor
    # < 1.0. Scaling contribution by it stops one continuing burst from counting
    # as many independent confirmations and artificially multiplying conviction.
    independence = _bounded(_first(data, ("independent_evidence_factor",
                                          "flow_excitation.independent_evidence_factor")), 1.0)
    signed = 1.0 if direction == "CALL" else -1.0 if direction == "PUT" else 0.0
    contribution = signed * score * reliability * freshness * weight * independence
    reason = str(_first(data, ("reason", "summary", "narrative", "label", "state", "regime", "bias", "value")) or directional or "Evidence available")
    if independence < 0.999:
        reason = f"{reason[:200]} · independence×{round(independence, 2)}"
    return EvidenceNode(engine, source, direction, round(score, 4), weight, round(freshness, 4), round(reliability, 4), round(contribution, 4), reason[:240], True, round(independence, 3))
