...
* Data reuse: historical outcomes come from the existing 23.4 store
  (``apex_learning_outcomes_v234``) via ``continuous_learning_calibration``;
...
try:  # Reuse the existing 23.4 outcome store; degrade gracefully if absent.
    from . import continuous_learning_calibration as learning_store  # type: ignore
...
# ── Route registration (absorbed from adaptive_confidence_calibration_routes.py, Sprint 3).
from flask import jsonify, request  # noqa: E402
from . import adaptive_confidence_calibration as calibration  # self-alias for absorbed route body
...
def register_adaptive_confidence_calibration_routes(app, *, last_result_provider=None):
    def current_payload():
