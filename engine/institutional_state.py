...
VERSION = "10.0.0_INSTITUTIONAL_STATE"
...
def _build_nodes(result: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        _node("liquidity", "Liquidity", state=liquidity_state, direction="NEUTRAL",
              confidence=_num(execution.get("liquidity_confidence")), available=liquidity_state != "UNKNOWN",
              evidence={"slippage": execution.get("slippage"), "spread_quality": execution.get("spread_quality")}),
        ...
        _node("quality", "Data Quality", state=gate_action, direction="NEUTRAL", confidence=quality_score,
              available=bool(quality or gate), evidence={"quality_score": quality_score, "multiplier": gate.get("multiplier")}),
...
def _story(result: Dict[str, Any], nodes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ii = _d(result.get("institutional_intelligence"))
