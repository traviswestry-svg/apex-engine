...
# ── Decision Narrative (absorbed from engine/decision_narrative.py, Sprint 2).
#    18.2.1 feature; sole consumer is the premium discipline stack.
NARRATIVE_VERSION = "18.2.1_DECISION_NARRATIVE"


def _narr_num(v, default=0.0):
    try:return float(v)
...
def build_decision_narrative(*, eligibility:Dict[str,Any], intelligence:Dict[str,Any], portfolio:Dict[str,Any], execution:Dict[str,Any], risk:Dict[str,Any], learning:Dict[str,Any]) -> Dict[str,Any]:
    ...
    if learning.get("best_pattern"):
        p=learning["best_pattern"]; evidence.append(f"Best learned analogue is {p.get('strategy')} in {p.get('regime')} with {p.get('samples')} samples, {round(_narr_num(p.get('win_rate'))*100,1)}% wins, and average P/L {p.get('average_pnl')}.")
