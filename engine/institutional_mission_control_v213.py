...
def build_mission_control(last:Dict[str,Any], configuration=None, dependencies=None)->Dict[str,Any]:
    ...
    return {'ok':True,'version':VERSION,'evaluated_at':datetime.now(timezone.utc).isoformat(),'state':overall,'groups':groups,
      'decision_banner':ws['decision_banner'],'regime_intelligence':{'primary_regime':regime.get('primary_regime'),'secondary_regime':regime.get('secondary_regime'),'confidence':regime.get('confidence'),'transition':regime.get('transition'),'risk_posture':regime.get('risk_posture')},'trading_brain':{'headline':brain.get('headline'),'primary_thesis':brain.get('primary_thesis'),'alternate_scenario':brain.get('alternate_scenario'),'confidence':brain.get('calibrated_confidence'),'conflicts':brain.get('conflicting_evidence'),'execution_readiness':brain.get('execution_readiness')},'execution_readiness':ws['execution_readiness'],'drilldowns':{
       'decision':'/api/institutional-decision/diagnostics','volume':'/api/institutional-volume-profile/diagnostics','execution':'/api/execution-optimizer/plan','strategy':'/api/strategy-intelligence/diagnostics','configuration':'/api/configuration/diagnostics','dependencies':'/api/dependencies/diagnostics','memory':'/api/market-memory/diagnostics','hardening':'/api/pre23-hardening/status','snapshot':'/api/institutional-snapshot/status','trading_brain':'/api/trading-brain/diagnostics','regime_intelligence':'/api/regime-intelligence/diagnostics'},
