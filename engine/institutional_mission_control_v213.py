...
def build_mission_control(last:Dict[str,Any], configuration=None, dependencies=None)->Dict[str,Any]:
    ...
    try: decision_integrity=decision_integrity_engine.evaluate_decision(last)
    except Exception: decision_integrity={'decision':{'execution_eligibility':'STAND_DOWN','integrity_adjusted_confidence':0},'evidence_health':{'state':'UNRELIABLE'}}
    try: institutional_reasoning=reasoning_engine.build_reasoning(last)
    except Exception: institutional_reasoning={'reasoning':{'execution_eligibility':'STAND_DOWN','trade_grade':{'grade':'F'},'final_confidence':0}}
    groups={
      ...
      'DECISION_INTEGRITY':{'state':'PASS' if decision_integrity.get('decision',{}).get('execution_eligibility')=='ELIGIBLE' else ('WARNING' if decision_integrity.get('decision',{}).get('execution_eligibility')=='WATCH' else 'BLOCKED'),'summary':f"{decision_integrity.get('decision',{}).get('execution_eligibility')} · {decision_integrity.get('decision',{}).get('integrity_adjusted_confidence')}% · evidence {decision_integrity.get('evidence_health',{}).get('state')}"},
      'INSTITUTIONAL_REASONING':{'state':'PASS' if institutional_reasoning.get('reasoning',{}).get('execution_eligibility')=='ELIGIBLE' else ('WARNING' if institutional_reasoning.get('reasoning',{}).get('execution_eligibility')=='WATCH' else 'BLOCKED'),'summary':f"{institutional_reasoning.get('reasoning',{}).get('trade_grade',{}).get('grade')} · {institutional_reasoning.get('reasoning',{}).get('final_confidence')}% · reasoning active"},
