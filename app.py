...
# APEX Trade Director Phase 14 — strategy orchestration and opportunity ranking
try:
    from engine.trade_director_strategy_orchestration import build_strategy_orchestration as td14_build_orchestration
    TRADE_DIRECTOR_PHASE14_AVAILABLE = True
except Exception as _td14_err:
...
# APEX 26.1-26.5 — Execution Intelligence Suite (required, advisory-only).
try:
    from engine.execution_suite_v26x_routes import (
        register_execution_suite_v26x_routes,
        verify_registered as _es26x_verify_registered,
        REQUIRED_ROUTES as _ES26X_REQUIRED,
    )
    EXECUTION_SUITE_V26X_AVAILABLE = True
except Exception as _es26x_err:
    register_execution_suite_v26x_routes = None  # type: ignore[assignment]
    _es26x_verify_registered = None  # type: ignore[assignment]
    _ES26X_REQUIRED = ()  # type: ignore[assignment]
...
def monitor_active_position() -> Dict[str, Any]:
    ...
    if TRADE_DIRECTOR_PHASE14_AVAILABLE:
        try:
            _td14_history = td6_trade_history(500) if TRADE_DIRECTOR_PHASE6_AVAILABLE else []
            result["strategy_orchestration"] = td14_build_orchestration(result, _td14_history)
    ...
    if TRADE_DIRECTOR_PHASE19_AVAILABLE:
        try:
            ...
            result["decision_intelligence"] = td19_build_decision_intelligence(result)
...
def tv_signal():
    ...
    # APEX 69.8.1 — capture every Pine trigger regardless of the assistant's
...
def api_institutional_os():
    ...
    if APEX_ENGINES_AVAILABLE and _build_institutional_decision is not None:
        try:
            ...
            # APEX 69.10.16 — preserve the exact scanner-owned gamma observation
            ...
            if isinstance(flow_snapshot, dict) and isinstance(flow_snapshot.get("gamma_transition"), dict):
                result["gamma_transition"] = dict(flow_snapshot["gamma_transition"])
...
@app.route("/api/position/institutional-decision-intelligence", methods=["GET", "POST"])
def api_position_institutional_decision_intelligence():
    """Return or evaluate Phase 19 cached-only institutional decision fusion."""
    if not TRADE_DIRECTOR_PHASE19_AVAILABLE:
        return jsonify({"ok": False, "error": "Phase 19 unavailable"}), 503
    monitor = monitor_active_position()
    if request.method == "POST":
        body = request.get_json(silent=True) or {}
        context = dict(monitor)
        context.update(body.get("context") or body.get("evidence") or {})
        data = td19_build_decision_intelligence(context)
...
try:
    ...
    if INSTITUTIONAL_DECISION_ENGINE_AVAILABLE and register_institutional_decision_engine_routes is not None:
        def _ide_last_result():
            with STATE_LOCK:
                value = STATE.get("last_result") or {}
                return dict(value) if isinstance(value, dict) else {}
    ...
    if EXECUTION_OS_ROUTES_AVAILABLE and register_execution_os_routes is not None:
        ...
        def _execution_os_risk_config():
            # Global (system-level) risk limits loaded at boot. Presence of these
            # limits is what the Morning Readiness "Risk" row reports as READY.
            cfg = {
...
if not EXECUTION_SUITE_V26X_AVAILABLE or register_execution_suite_v26x_routes is None:
    ...
    def _es26x_last_result():
        with STATE_LOCK:
            ...
            return dict(value) if isinstance(value, dict) else {}
    try:
        register_execution_suite_v26x_routes(app, last_result_provider=_es26x_last_result)
    except (AssertionError, ValueError) as e:
        raise RuntimeError(
            "APEX 26.1-26.5 Execution Intelligence Suite route registration failed "
            f"(likely a duplicate route or endpoint conflict): {e}") from e
    _es26x_missing = _es26x_verify_registered(app)
    if _es26x_missing:
        raise RuntimeError(
            "APEX 26.1-26.5 Execution Intelligence Suite failed to register required "
...
def api_admin_storage_audit():
    """APEX 69.10.8 read-only persistent-storage capacity-governance audit.
    ...
    try:
        ...
        payload["schema_version"] = "apex.storage_audit.v2"
        ...
        app.logger.exception("APEX 69.10.8 storage audit failed")
        return jsonify({
            "ok": False,
            "schema_version": "apex.storage_audit.v2",
